...
let tests =
  ...
  >::: [
         ...
         ( "given several average rolls then score is the sum of these rolls"
         >:: fun _ ->
         ...
         ( "given a strike then the two next rolls are added as bonus"
         >:: fun _ ->
         ...
         ( "after 10 frames rolls are not added, only bonus" >:: fun _ ->
           assert_equal ~printer:string_of_int 300
